...
int main(void)
{
	...
	DIO2_Init(0xFF, DIO_MODE_OUT);		// **DIO2のすべてのピンを出力として初期化する．
	...
	while(1)
	{
		...
		DIO_TogglePin(2, 0);	// DIO2-0の出力を反転させる．
		DIO_TogglePin(2, 1);	// DIO2-1の出力を反転させる．
		DIO_TogglePin(2, 2);	// DIO2-2の出力を反転させる．
		DIO_TogglePin(2, 3);	// DIO2-3の出力を反転させる．
		DIO_TogglePin(2, 4);	// DIO2-4の出力を反転させる．
		DIO_TogglePin(2, 5);	// DIO2-5の出力を反転させる．
		DIO_TogglePin(2, 6);	// DIO2-6の出力を反転させる．
		DIO_TogglePin(2, 7);	// DIO2-7の出力を反転させる．
