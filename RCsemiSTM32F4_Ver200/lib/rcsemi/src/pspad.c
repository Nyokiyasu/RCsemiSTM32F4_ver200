/***************************************************************************
 *	@ファイル名		:	pspad.c
 *	@概要		:	電話線, BluetoothにおけるPSコントローラとの通信を
 *					提供する．
 *	@バージョン		:	2.0.0
 *	@開発者		:	キネクト
...
/* -------------------------------------------------
 * @関数名		:	Modular_BDMaster_Init
 * @概要			:	電話線との通信で
 * 					自分をマスターとして初期化
 * @引数-command	:	初期化時に自動返信する文字列
 * @戻り値		:	なし
   ---------------------------------------------- */
void Modular_BDMaster_Init(char *command)
{
	char buff[128];
	Modular_RecvString(buff, 128);
	while(!Coincidence_Check(buff, "OK", 2))
	{
		Modular_RecvString(buff, 128);
	}
...
/* -------------------------------------------------
 * @関数名		:	Modular_BDSlave_Init
 * @概要			:	電話線との通信で
 * 					自分をスレーブとして初期化
 * @引数-command	:	初期化時に自動返信する文字列
 * @戻り値		:	なし
   ---------------------------------------------- */
void Modular_BDSlave_Init(char *command)
{
	char buff[128];
	Modular_RecvString(buff, 128); //"OK"を受けたはず
	while(!Coincidence_Check(buff, "OK", 2))
	{
		Modular_RecvString(buff, 128);
	}
...
/* -------------------------------------------------
 * @関数名	:	Modular_PSpacket_Recv
 * @概要		:	電話線から
 * 				PSコントローラのパケット(7Byte)を受け取る
 * @引数-buf	:	受け取った情報を格納する変数
 * @戻り値	:	情報が受け取れれば7を、そうでなければ-1を返す
   ---------------------------------------------- */
static int Modular_PSpacket_Recv(u8 buf[])
{
	int data, time0;
	int i;

	/* 理由は不明だがdo-while文は利用できなかった */
	data = Modular_RecvByte();
	if(data == -1)	return -1;
...
/* -------------------------------------------------
 * @関数名		:	Modular_PSdata_Recv
 * @概要			:	USART3(電話線)から
 * 					PSコントローラのデータを受信する．
 * @引数-psdata	:	コントローラデータ格納先の変数
 * @戻り値		:	なし
 * ---------------------------------------------- */
void Modular_PSdata_Recv(psdata_format_t *psdata)
{
	int i;
	static int mode;
	int nBytes, sumchk = -1;
	nBytes = Modular_PSpacket_Recv(psdata->bytes);
	if(nBytes == 7)	sumchk = PSdata_CheckSum(psdata);
	if(sumchk == 1)
	{
		mode = psdata->name.Mode;
